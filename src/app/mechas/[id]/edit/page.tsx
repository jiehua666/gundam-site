...
export default function EditMechaPage() {
  ...
  const fetchMecha = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setName(mecha.name || "");
        setSeries(mecha.series || "");
        setGrade(mecha.grade || "");
        setClassification(mecha.classification || "");
        setCoverImage(mecha.coverImage || "");
        setSummary(mecha.summary || "");
        setHeight(mecha.height || "");
        setWeight(mecha.weight || "");
        setPowerSystem(mecha.powerSystem || "");
        setArmor(mecha.armor || "");
        setSpecs(mecha.specs?.map((s: { specKey: string; specValue: string }) => ({ specKey: s.specKey, specValue: s.specValue })) || []);
        setPalettes(mecha.palettes?.map((p: { name: string; primaryColor: string | null; secondaryColor: string | null; accentColor: string | null }) => ({
          name: p.name,
      ...
      console.error("Failed to fetch mecha:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (user.role !== 'admin' && user.role !== 'founder') {
    return (
      <div className="bg-background min-h-screen flex flex-col items-center justify-center">
        <p className="text-destructive mb-4">只有管理员可以编辑机体</p>
