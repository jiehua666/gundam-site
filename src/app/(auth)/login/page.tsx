...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUser(result.user);
      toast.success(`Welcome back, ${result.user.nickname}!`);
        ...
        <div className="glass-card neon-border rounded-xl p-8">
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-muted-foreground">
              ...
              <Link href="/register" className="text-primary hover:text-primary/80 font-medium neon-glow">
                Register
              </Link>
            </p>
          </div>
          ...
          <div className="mt-8 pt-6 border-t border-border">
            <p className="text-xs text-center text-muted-foreground">
              Test accounts: admin / testuser (password: admin123)
            </p>
          </div>
        </div>
      </div>
